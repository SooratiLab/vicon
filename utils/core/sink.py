...
Unified output handler for tracking data (Vicon, TurtleBot localization, etc.).
...
class TurtleBotPosition:
    """Position data for a single TurtleBot (for backward compatibility)."""
    name: str
    local_id: int
    x: float
    y: float
...
class ReferenceMarkerPosition:
    """Position data for a reference marker (for backward compatibility)."""
...
class DataSink:
    """
    Unified sink for tracking data.
    
    Publishes to both CSV writer and broadcaster if configured.
    Supports both Vicon data and TurtleBot localization data.
    """
    
    def __init__(
        self,
        csv_writer: Optional["ViconCSVWriter"] = None,
    ...
    def publish_vicon(self, vicon_data: Dict[str, Any]):
        """
        Publish Vicon tracking data.
        
        Args:
            vicon_data: Dictionary containing Vicon frame data
        """
        if self._paused:
            return
        ...
        # Write to CSV
        if self.csv_writer:
            self.csv_writer.write_snapshot(vicon_data)
        
        # Broadcast
        if self.broadcaster:
            self.broadcaster.update(vicon_data)
    ...
    def publish(
        ...
        """
        Publish TurtleBot localization data (for backward compatibility).
        ...
        # For TurtleBot data, we only broadcast (CSV writer expects Vicon format)
        if self.broadcaster:
